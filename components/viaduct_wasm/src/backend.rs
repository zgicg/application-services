...
// testing, for example.
static FFI_FORCED: AtomicBool = AtomicBool::new(true);
...
pub async fn send(request: crate::Request) -> Result<crate::Response, crate::Error> {
    validate_request(&request)?;
    if ffi_is_forced() {
        return self::ffi::send(request).await;
    }
    #[cfg(feature = "reqwest")]
    {
        self::reqwest::send(request).await
    }
    #[cfg(not(feature = "reqwest"))]
    {
        self::ffi::send(request).await
    }
}
