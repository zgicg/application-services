...
use ffi_support::{
    ...
    ConcurrentHandleMap, Handle, HandleError, ExternError, FfiStr,
};
use fxa_client::{
...
pub fn fxa_new(
    ...
    let handle = ACCOUNTS.insert_with_output(&mut err, || {
        FirefoxAccount::new(content_url, client_id, redirect_uri)
    });
    return handle; // XXX TODO: Result
}
...
/// pointer type.
#[wasm_bindgen]
pub fn fxa_begin_oauth_flow(
    handle: u64,
    scope: &str,
) -> js_sys::Promise {
    //log::debug!("fxa_begin_oauth_flow");
    let scope = "profile test";
    let scopes: Vec<&str> = scope.split(' ').collect();
    let f = async move {
        ACCOUNTS.get_mut_u64_async(handle, |fxa| {
            let mut fxa = fxa.clone();
            Box::new((async move || -> Result<JsValue, HandleError> {
               let v = match(fxa.begin_oauth_flow(&scopes).await) {
                   Ok(s) => s,
                   Err(e) => format!("ERROR: {:?}", e),
               };
               Ok(JsValue::from_str(&v))
            })().boxed_local())
        }).await.map_err(|e| JsValue::from_str(&format!("ERROR: {:?}", e)))
    };
    //let f = future::ok::<JsValue, JsValue>(JsValue::null());
    return future_to_promise(f.boxed_local().compat());
}
/*
