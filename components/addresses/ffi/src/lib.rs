...
pub extern "C" fn sync15_passwords_add(
    ...
    ENGINES.call_with_result(error, handle, |state| {
        ...
        let address: Address = serde_json::from_value(parsed)?;
        state.lock().unwrap().add(address)
    })
}
...
pub extern "C" fn sync15_passwords_import(
    ...
    ENGINES.call_with_result(error, handle, |state| {
        let addresses: Vec<Address> = serde_json::from_str(records_json.as_str())?;
        state.lock().unwrap().import_multiple(&addresses)
    })
}
...
pub extern "C" fn sync15_passwords_update(
    ...
    ENGINES.call_with_result(error, handle, |state| {
        let parsed: Address = serde_json::from_str(record_json.as_str())?;
