...
use crate::{Error, ErrorKind, Address};
...
fn get_code(err: &Error) -> ErrorCode {
    match err.kind() {
        ...
        ErrorKind::InvalidAddress(desc) => {
            log::error!("Invalid address: {}", desc);
            ErrorCode::new(error_codes::INVALID_ADDRESS)
        }
...
implement_into_ffi_by_json!(Address);
