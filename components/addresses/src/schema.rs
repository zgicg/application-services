...
//! ================
//!
//! The schema we use is a evolution of the firefox-ios addresses database format.
//! There are three tables:
//!
//! - `addressesL`: The local table.
//! - `addressesM`: The mirror table.
//! - `addressesSyncMeta`: The table used to to store various sync metadata.
//!
//! ## `addressesL`
//!
//! This stores local address information, also known as the "overlay".
//!
//! `addressesL` is essentially unchanged from firefox-ios, however note the
//! semantic change v4 makes to timestamp fields (which is explained in more
//! detail in the [COMMON_COLS] documentation).
//!
//! It is important to note that `addressesL` is not guaranteed to be present for
//! all records. Synced records may only exist in `addressesM` (although this is
//! not guaranteed). In either case, queries should read from both `addressesL` and
//! `addressesM`.
//!
//! ### `addressesL` Columns
//!
...
//! ## `addressesM`
//!
//! This stores server-side address information, also known as the "mirror".
//!
//! Like `addressesL`, `addressM` has not changed from firefox-ios, beyond the
//! change to store timestamps as milliseconds explained in [COMMON_COLS].
//!
//! Also like `addressesL`, `addressesM` is not guaranteed to have rows for all
//! records. It should not have rows for records which were not synced!
//!
//! It is important to note that `addressesL` is not guaranteed to be present for
//! all records. Synced records may only exist in `addressesM`! Queries should
//! test against both!
//!
//! ### `addressesM` Columns
//!
...
//!   are invalid, and that we should defer to the data stored in `addressesL`.
//!
//! ## `addressesSyncMeta`
//!
...
/// further confusion.
///
/// However, note that the `local_modified` (of `addressesL`) and `server_modified`
/// (of `addressesM`) are stored as milliseconds as well both on firefox-ios and
...
lazy_static! {
    static ref CREATE_LOCAL_TABLE_SQL: String = format!(
        "CREATE TABLE IF NOT EXISTS addressesL (
            {common_sql},
    ...
    static ref CREATE_MIRROR_TABLE_SQL: String = format!(
        "CREATE TABLE IF NOT EXISTS addressesM (
            {common_sql},
...
const CREATE_META_TABLE_SQL: &str = "
    CREATE TABLE IF NOT EXISTS addressesSyncMeta (
...
const CREATE_OVERRIDE_HOSTNAME_INDEX_SQL: &str = "
    CREATE INDEX IF NOT EXISTS idx_addressesM_is_overridden_hostname
    ON addressesM (is_overridden, hostname)
";

const CREATE_DELETED_HOSTNAME_INDEX_SQL: &str = "
    CREATE INDEX IF NOT EXISTS idx_addressesL_is_deleted_hostname
    ON addressesL (is_deleted, hostname)
";
...
// https://github.com/mozilla-mobile/firefox-ios/blob/master/Storage/SQL/AddressesSchema.swift#L100
fn upgrade(db: &Connection, from: i64) -> Result<()> {
    ...
    if from < 4 {
        ...
        // The `addressesSyncMeta` table was added in v4, and we moved
...
pub(crate) fn drop(db: &Connection) -> Result<()> {
    ...
    db.execute_all(&[
        "DROP TABLE IF EXISTS addressesM",
        "DROP TABLE IF EXISTS addressesL",
        "DROP TABLE IF EXISTS addressesSyncMeta",
