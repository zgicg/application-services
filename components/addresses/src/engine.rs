...
impl AddressEngine {
    ...
    pub fn get(&self, id: &str) -> Result<Option<Address>> {
    ...
    pub fn update(&self, address: Address) -> Result<()> {
        self.db.update(address)
    }

    pub fn add(&self, address: Address) -> Result<String> {
        // Just return the record's ID (which we may have generated).
        self.db.add(address).map(|record| record.guid.into_string())
    }

    pub fn import_multiple(&self, addresses: &[Address]) -> Result<u64> {
        self.db.import_multiple(addresses)
    }
...
mod test {
    ...
    fn test_general() {
        ...
        let a = Address {
            ...
            form_submit_url: Some("https://www.example.com/address".into()),
        ...
        assert_addresses_equiv(&a, &a_from_db);
        ...
        assert_addresses_equiv(&b_after_update, &b2);
