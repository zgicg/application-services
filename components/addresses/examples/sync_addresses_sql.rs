...
use addresses::{Address, AddressesEngine};
...
fn update_address(record: &mut Address) {
...
fn show_sql(e: &AddressesEngine, sql: &str) -> Result<()> {
...
fn show_all(engine: &AddressesEngine) -> Result<Vec<Guid>> {
...
fn prompt_record_id(e: &AddressesEngine, action: &str) -> Result<Option<String>> {
...
fn main() -> Result<()> {
    ...
    let matches = clap::App::new("sync_pass_sql")
        .about("CLI address syncing tool (backed by sqlcipher)")
        .arg(
            clap::Arg::with_name("database_path")
                .short("d")
                .long("database")
                .value_name("ADDRESSES_DATABASE")
                .takes_value(true)
                .help("Path to the addresses database (default: \"./addresses.db\")"),
        )
        .arg(
    ...
    let db_path = matches.value_of("database_path").unwrap_or("./addresses.db");
    ...
    let engine = AddressesEngine::new(db_path, Some(encryption_key))?;

    log::info!("Engine has {} passwords", engine.list()?.len());

    if let Err(e) = show_all(&engine) {
        log::warn!("Failed to show initial address data! {}", e);
    }
        ...
        match prompt_char("[A]dd, [D]elete, [U]pdate, [S]ync, [V]iew, [H]ostname search, [R]eset, [W]ipe, [T]ouch, E[x]ecute SQL Query, or [Q]uit").unwrap_or('?') {
            ...
            'U' | 'u' => {
                ...
                match prompt_record_id(&engine, "update") {
                    ...
                    Ok(Some(id)) => {
                        let mut address = match engine.get(&id) {
                            Ok(Some(address)) => address,
                            Ok(None) => {
                                log::warn!("No such address!");
                                continue
                            }
                            Err(e) => {
                        ...
                        update_address(&mut address);
                        if let Err(e) = engine.update(address) {
