...
open class AddressRecord {
    ...
    /// result in an `AddressesStoreError.InvalidAddress`.
    public var httpRealm: String?

    /// The submission URL for the form where this address may be entered.
    ///
    ...
    /// both or neither will result in an `AddressesStoreError.InvalidAddress`.
    ...
    public static func fromJSONArray(_ jsonArray: String) throws -> [AddressRecord] {
        if let arr = try JSONSerialization.jsonObject(with: jsonArray.data(using: .utf8)!,
                                                      options: []) as? [[String: Any]] {
            return arr.map { (dict) -> AddressRecord in
                AddressRecord(fromJSONDict: dict)
            }
        }
